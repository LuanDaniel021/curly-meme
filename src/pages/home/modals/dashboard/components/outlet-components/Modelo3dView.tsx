...
function Modelo3dView() {
    ...
    const fetchVehicleByPlate = async (plateValue: string) => {
        const normalizedPlate = plateValue
            .replace(/[^a-zA-Z0-9]/g, "")
            .toUpperCase();
        ...
        try {
            const response = await fetch(
                `/api/crlvs/placa/${encodeURIComponent(normalizedPlate)}`,
            );
            if (!response.ok) {
                ...
                throw new Error(
                    errorBody?.message ??
                        `Não foi possível consultar a placa (${response.status}).`,
                );
            }
    ...
    useEffect(() => {
        ...
        const floorGeometry = new THREE.CylinderGeometry(6, 6, 0.25, 64);
        ...
        mtlLoader.setPath("/model/caminhao_4_rodas_OBJ/");
        ...
        mtlLoader.load(
            ...
            (materials) => {
                ...
                objLoader.setPath("/model/caminhao_4_rodas_OBJ/");
                ...
                objLoader.load(
                    ...
                    (object) => {
                        ...
                        console.log("Caminhão carregado corretamente!");
                    },
    ...
    const handlePointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
        if (!draggingRef.current || !truckRef.current) {
            return;
        }

        const currentX = event.clientX;

        const difference = currentX - previousXRef.current;
        ...
        truckRef.current.rotation.y += difference * 0.008;
    };
    ...
    const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
        ...
        const limitedZoom = Math.max(5, Math.min(14, newZoom));

        setZoom(limitedZoom);

        if (cameraRef.current) {
            cameraRef.current.position.z = limitedZoom;
    ...
    const recenterTruck = () => {
        ...
        if (cameraRef.current) {
            cameraRef.current.position.set(0, 3.2, 10);
    ...
    const handleTruckClick = (event: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (intersections.length > 0) {
            ...
            while (
                selectedObject &&
                selectedObject.userData.tireIndex === undefined
            ) {
                selectedObject = selectedObject.parent;
            }

            const tireIndex = selectedObject?.userData.tireIndex as
                | number
                | undefined;
            if (tireIndex !== undefined) {
    ...
    if (activeTireScreen) {
        return (
            <div className="app-shell">
                <TireDetailsScreen
                    tire={activeTireScreen}
                    plate={plate}
                    vehicleData={vehicleData}
                    onBack={() => setActiveTireScreen(null)}
                    onTireUpdated={(updatedTire) => {
                        setTruckTires((current) =>
                            current.map((item) =>
                                item.id === updatedTire.id ? updatedTire : item,
                            ),
                        );
                        setActiveTireScreen(updatedTire);
                    }}
                />
            </div>
        );
                ...
                <header className="topbar">
                    <div className="brand">
                        ...
                        <div>
                            ...
                            <span>
                                Caminhão 4 Rodas • Troca, Recapagens e Vida Útil
                            </span>
                        </div>
                    </div>
      ...
      ================================================= */}
                    ...
                    <div className="plate-area">
                        ...
                        <div className="plate">
                            ...
                            <input
                                ...
                                onKeyDown={(event) => {
                                    if (event.key === "Enter")
                                        void fetchVehicleByPlate(plate);
                                }}
                            />
                            <span className="plate-country">BR</span>
                        </div>
                        {plateLoading && (
                            <span className="plate-feedback">
                                Consultando...
                            </span>
                        )}
                        {plateError && (
                            <span className="plate-feedback plate-error">
                                {plateError}
                            </span>
                        )}
                        {vehicleData !== null && (
                            <span className="plate-feedback plate-success">
                                Veículo localizado
                            </span>
                        )}
                        {(plateLoading ||
                            ...
                            <section
                                ...
                                aria-live="polite"
                            >
                                <div className="plate-response-header">
                                    <span>Resposta da consulta</span>
                                    <strong>{plate}</strong>
                                </div>
                                {plateLoading ? (
                                    <p className="plate-response-state">
                                        Consultando dados...
                                    </p>
                                ) : plateError ? (
                                    <p className="plate-response-state plate-error">
                                        {plateError}
                                    </p>
                                ) : (
                                    <pre>{formatApiResponse(vehicleData)}</pre>
                                )}
                            </section>
                        )}
                    </div>
        ...
        ================================================= */}
                    ...
                    <div className="truck-controls">
                        <button onClick={recenterTruck}>↻ Recentrar</button>

                        <span>
                            Arraste para a esquerda ou direita para girar
                        </span>
                    </div>
        ...
        ================================================= */}
                    ...
                    <div className="help-box">
                        ...
                        <p>
                            Arraste o caminhão para a esquerda ou direita para
                            visualizar os lados.
                        </p>
                        ...
                        <div className="help-items">
                            ...
                            <span>🖱 Rodinha para zoom</span>

                            <span>
                                🛞 Clique no pneu para abrir as informações
                            </span>
                        </div>
                    </div>
                </main>
      ...
      ================================================= */}
                ...
                {selectedTire && (
                    <aside className="tire-drawer">
                        ...
                        <h2>{selectedTire.position}</h2>

                        <span className="status">{selectedTire.status}</span>

                        <div className="life">
                            <div className="life-header">
                                <span>Vida útil estimada</span>

                                <strong>{selectedTire.life}%</strong>
                            </div>

                            <div className="life-bar">
                                <div
                                    style={{
                                        width: `${selectedTire.life}%`,
                                    }}
                                />
                            </div>
                        </div>
                        ...
                        <div className="tire-data">
                            <div>
                                ...
                                <strong>{selectedTire.pressure}</strong>
                            </div>

                            <div>
                                <span>◉ Sulco</span>

                                <strong>{selectedTire.tread}</strong>
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            type="button"
                            onClick={() => {
                                setActiveTireScreen(selectedTire);
